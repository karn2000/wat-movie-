...
class Main(object):
    def __init__(self):
        ...
        m_search = Button(self.root, text = 'SEARCH', bg='#5fbfc5',command = lambda : self.d_movie(label2, ment, option1)).place(relx=.5, rely=.4, anchor= 'c')
    ...
    def changelabel(self, before, after, check):
        ...
        label = Label(self.root, text = movie,image=ooo).place(relx=.69, rely=.63)
        self.tkpic()

    def d_movie(self, label2, ment, option1):
        ''' find movie '''
        movie = ment.get()
                ...
                for l in movie_list[i]:
                    if movie.lower() in l:
                        ...
                        if check > 1:
                            movie_1 += '\n'
                        movie_1 += l
        self.changelabel(label2, movie_1, check)
    def tkpic(self):
        pic = Tk()
        m_minigame = Button(pic, text = 'Mini Game', command = openmini, bg = 'yellow').grid(row=0,column=1, sticky=W)
        quit_button = Button(pic, text = 'Close',bg = 'pink', command = self.mquit).grid(row=0,column=0, sticky=W)
        pic.mainloop()
